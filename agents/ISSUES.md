...
```
issues/
├── cloud-sync-integration.md    # Epic (high-level overview)
├── dropbox-01-sdk-setup.md      # Ticket (specific task)
├── dropbox-02-oauth-pkce.md
├── icloud-01-entitlements.md
└── ...
```
...
- **File naming**: `feature-NN-short-description.md` (e.g., `dropbox-02-oauth-pkce.md`)
...
5. Run `make check` before marking complete
6. Update status to `Complete`

### Creating new tickets

1. If working on an existing epic, follow its naming pattern (e.g., `dropbox-05-...`)
...
1. **Implicit (number order)**: `dropbox-01` should be done before `dropbox-02`
2. **Explicit**: Listed in the Dependencies field when non-linear

Example:
```markdown
**Dependencies:** dropbox-01-sdk-setup, icloud-01-entitlements
```
